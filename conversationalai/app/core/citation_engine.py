...
class CitationQueryEngine:
    ...
    def prepare_context_with_sources(self, chunks: list) -> tuple:
        """Assign integer IDs starting at 1 to each chunk.

        Returns:
            (formatted_context, source_map)
            - formatted_context: str with "[Source 1]: …" blocks
        ...
        for idx, chunk in enumerate(chunks, start=1):
            parts.append(f"[Source {idx}]: {chunk.raw_text}")
            source_map[idx] = chunk.chunk_id
    ...
    def parse_source_markers(
        ...
        """Extract all ``[Source N]`` markers from *text*.

        Returns a deduplicated list of ``(source_number, chunk_id)`` tuples.
        Source numbers not present in *source_map* are logged and skipped.
        """
        raw_numbers = re.findall(r"\[Source (\d+)\]", text)
        ...
        for raw in raw_numbers:
            ...
            if num not in source_map:
                logger.warning(
                    "Source marker [Source %d] not found in source_map — skipping",
                    num,
                )
                continue
...
class SentenceSplitter:
    ...
    def map_citations_to_sentences(
        ...
        """For each sentence, find all [Source N] markers.

        Returns:
            [{"sentence": str, "source_numbers": [int, …], "chunk_ids": [str, …]}, …]
        """
        results: list[dict] = []

        for sentence in sentences:
            nums = [int(n) for n in re.findall(r"\[Source (\d+)\]", sentence)]
            chunk_ids = [
...
class BoundingBoxMapper:
    ...
    def __init__(
        ...
        self.fallback_chunks = {
            getattr(chunk, "chunk_id", ""): chunk
            for chunk in (fallback_chunks or [])
        }

    def _bbox_from_chunk(self, chunk_id: str) -> BoundingBox | None:
        """Use chunk-local bounding boxes when Qdrant is unavailable."""
        chunk = self.fallback_chunks.get(chunk_id)
        if chunk is None:
            return None

        bbox_data = getattr(chunk, "bounding_box", None)
        if bbox_data is None and hasattr(chunk, "model_dump"):
            bbox_data = chunk.model_dump().get("bounding_box")
        ...
        try:
            if isinstance(bbox_data, BoundingBox):
                return bbox_data
            if hasattr(bbox_data, "model_dump"):
            ...
            return BoundingBox(
                ...
                page=int(bbox_data.get("page", getattr(chunk, "page_number", 0))),
            )
        except (TypeError, ValueError, AttributeError):
            logger.info("Ignoring malformed fallback bounding_box for chunk %s", chunk_id)
            return None

    async def get_bounding_box(self, chunk_id: str) -> BoundingBox:
        """Fetch the bounding box for a single chunk from Qdrant."""
        fallback_bbox = self._bbox_from_chunk(chunk_id)

        if self.qdrant_client is None:
            return fallback_bbox or BoundingBox(x=0, y=0, w=0, h=0, page=0)
        ...
        try:
            ...
            logger.info("Falling back after Qdrant lookup failed for chunk %s", chunk_id)
            return fallback_bbox or BoundingBox(x=0, y=0, w=0, h=0, page=0)
