...
[YOU] Docling Parser → Structural Chunker → Contextual Enricher
    → Metadata Extractor → Late Chunking (Jina AI) → Dual Indexer (Qdrant + BM25)
    ↓                          ↓
...
| `dual_indexer.py` | `DualIndexer` | Chunks → Qdrant (dense, via Jina AI) + BM25 (sparse) |
...
List of FinalChunks  (+ chunk_id, chunk_index, project_id, file_id, page_number, bounding_box)
  ...
  → Qdrant: Late Chunking via Jina AI 8k transformer → 1536-dim vector + full payload
...
- **Late Chunking for Vectors:** Pass the broader document context through the Jina AI 8k transformer before creating the vector embeddings to preserve semantic meaning.
...
6. `ContextualEnricher` calls GPT-4o-mini / Gemini 2.0 Flash:
...
8. `DualIndexer` (parallel):
   - Embeds `enriched_text` → Jina AI (Late Chunking) → OpenAI `text-embedding-3-large` → Qdrant.
...
- **Vector:** 1536 dimensions, Cosine distance.
...
- Update `DualIndexer` — implement Jina AI 8k Late Chunking before OpenAI embedding.
...
- OpenAI API timeout during enrichment → degrade gracefully (skip summary, don't crash)
...
- Required Env Vars: `OPENAI_API_KEY`, `GEMINI_API_KEY` (or `ANTHROPIC_API_KEY`), `QDRANT_HOST`, `QDRANT_PORT`, `REDIS_URL`, `DATABASE_URL`, `BM25_INDEX_DIR`, `PDF_UPLOAD_DIR`.
...
| OpenAI API down (enrichment) | Retry 3× with backoff → if still failing, store chunk without summary |
