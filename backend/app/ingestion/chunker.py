...
class StructuralChunker:
    ...
    def chunk_document(self, docling_result, file_id: str = "") -> List[RawChunk]:
        ...
        for item, _level in doc.iterate_items():
            item_type = getattr(item, '_item_type', None) or type(item).__name__
            ...
            if item_type in ("SectionHeaderItem", "SectionItem"):
                ...
                table_text = self._get_text(item, doc)
                ...
                caption = self._get_text(item, doc)
                chunks.append(RawChunk(
    ...
    def _get_text(self, item, doc) -> str:
        ...
        if hasattr(item, "export_to_markdown"):
            try:
                # Newer Docling versions require 'doc' for some items (like PictureItem)
                return item.export_to_markdown(doc=doc) or ""
            except TypeError:
                # Fallback for items that don't take 'doc'
    ...
    def _export_table_html(self, table_item, doc) -> Optional[str]:
        ...
        try:
            if hasattr(table_item, "export_to_html"):
                return table_item.export_to_html(doc=doc)
            ...
            text = self._get_text(table_item, doc)
            if text:
